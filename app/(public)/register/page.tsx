...
// Trang Đăng ký tài khoản mới DailyE
export default function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterInput) => {
    ...
    if (!result.success) {
      ...
      // Đăng ký thành công -> Chuyển hướng sang trang Onboarding
  ...
  const handleGoogleLogin = async () => {
    ...
    if (result && !result.success) {
      setServerError(result.error || 'Đăng ký Google thất bại');
    ...
    <div className="min-h-screen bg-slate-900 flex flex-col justify-between p-4 relative overflow-hidden">
      ...
      <main className="max-w-md w-full mx-auto bg-slate-800/80 border border-slate-700 backdrop-blur-xl rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6 my-auto">
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-slate-400">Học & luyện thi TOEIC hoàn toàn miễn phí cùng DailyE</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1">Mật khẩu (tối thiểu 8 ký tự)</label>
            <input
              type="password"
              placeholder="••••••••"
              {...register('password')}
